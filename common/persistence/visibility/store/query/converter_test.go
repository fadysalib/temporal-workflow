		...
		},
		{
			...
			setupMock: func() {
				...
				s.storeQCMock.EXPECT().
					...
					Return(e2, nil)
				s.storeQCMock.EXPECT().BuildAndExpr(e1, e2).Return(e1e2, nil)
			},
			mockNamespaceIDExpr: true,
			mockParenQueryExpr:  true,
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.mockParenQueryExpr {
				s.storeQCMock.EXPECT().BuildParenExpr(tc.inExpr).
...
func (s *queryConverterSuite) TestResolveSearchAttributeAlias() {
	testCases := []struct {
		name                 string
		in                   string
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.useNoopMapper {
				s.queryConverter.saMapper = searchattribute.NewNoopMapper()
			}

			fn, ft, err := s.queryConverter.resolveSearchAttributeAlias(tc.in)
			if tc.err != "" {
				s.Error(err)
				s.ErrorContains(err, tc.err)
				var expectedErr *ConverterError
...
func (s *queryConverterSuite) TestValidateValueType() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			out, err := s.queryConverter.validateValueType(tc.saName, tc.saType, tc.value)
			if tc.err != "" {
				s.Error(err)
				s.ErrorContains(err, tc.err)
				var expectedErr *ConverterError
